/**
 * Replay the source-pinned Field occurrences against the canonical Figma
 * exports without rewriting the feature proof directory.  The comparison uses
 * the same renderer, raw/region scoring, slot override validation and
 * geometry receipts as the campaign runner.  Its layout context is rebuilt
 * only from the prior pinned GET geometry receipt.
 */
import { readFileSync } from 'node:fs';
import path from 'node:path';
import { composeSubject } from '../../extract/figma/visual-parity/compose.js';
import { createGeometryReceipt, createPixelReceipt } from '../../extract/figma/visual-parity/evidence.js';
...
const cases = CAMPAIGN.subjects.flatMap((subject: any) =>
  subject.cases
    .filter((campaignCase: any) => TARGETS.has(campaignCase.id))
    .map((campaignCase: any) => ({ subject, campaignCase })),
);
if (cases.length !== TARGETS.size) {
...
try {
  ...
  for (const { subject, campaignCase } of cases) {
    ...
    if (!paritySubject) throw new Error(`${campaignCase.id}: no Field parity subject`);
    ...
    const contextRootWidth = campaignCase.layoutContext?.rootWidth === 'figma-root'
      ...
      : undefined;
    const contextPartWidths = Object.fromEntries((campaignCase.layoutContext?.partWidths ?? []).map((partName: string) => {
      const part = prior.geometry.parts[partName]?.figma;
      if (!part) throw new Error(`${campaignCase.id}: no pinned geometry for contextual part ${partName}`);
      return [partName, part.width * prior.geometry.rootFigma.width / 2];
    }));
    ...
    const aligned = alignPair(generated, figma, rendered.rootRect, campaignCase.comparisonSurface ?? 'light');
    ...
    const regionsText = Object.values(pixels.regions)
      .map((region: any) => `${region.id}=${region.diffPct.toFixed(3)}%`)
      .join(', ');
    console.log(`${campaignCase.id}: raw ${pixels.rawPct.toFixed(3)}%; ${regionsText}; geometry ${geometry.verdict}`);
    if (pixels.verdict !== 'pass' || geometry.verdict !== 'pass') {
      failures.push(`${campaignCase.id}: ${JSON.stringify({ pixels, geometry })}`);
    }
  }
} finally {
  await browser.close();
...
if (failures.length > 0) throw new Error(`Field targeted receipts failed:\n${failures.join('\n')}`);
