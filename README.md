...
All of it is gated by **97 executable checks** (`npm run eval`) that run the real pipeline in a scratch copy — not mocks. 94 pass; 3 are an intentional red block awaiting the Piqueray token-set push to Figma (see [MILESTONES.md](MILESTONES.md)). `npm run eval` prints the live `N/N` — trust that over any number quoted here.
...
| `evals/` | 97 deterministic checks on the machinery itself (94 passing, 3 intentional pending the Figma push): byte-identical regeneration against golden manifests, refusal of illegal contracts, detection of every claimed drift class, convergence after promotion, extraction round-trips. | ✅ |
...
npm run eval     # ④ 97 checks that detection, refusal, and convergence still hold (94/97 — 3 intentional)
```
