...
| **Governed geometry** | every layout dimension in every contract is either a governed token reference or a **named literal** declared in a closed registry the gate reads live — a raw number that no axis watches is refused by name. Zero invisible values across the 34 contracts; the trait, paint and typography literals stay out of scope and are counted as such, never folded into the zero | `npm run geometry:gate` · `geometry-gate-policy-check` eval |
| **One box model, four surfaces** | the delivered React library measures the same boxes as Figma and the other three surfaces — a part binding a size *and* a padding no longer renders wider for consumers than for the designer | `react-box-model-border-box` eval · [`box-model-unification.md`](specs/015-geometrie-gouvernee/proofs/recus/box-model-unification.md) |
| **Bounded gradient channel** | `background-image` carries a `linear-gradient(...)` literal on all four surfaces; `radial-`/`conic-gradient` refuse at schema validation, by name — the channel's own grammar, never a widening of the shared one | `gradient-literal-channel` eval · [docs/02](docs/02-contract-spec.md) |
| **Hand-set corrections survive** | a re-extraction that would silently overwrite a value set by hand in a previous campaign is detected and refused — the clobber reproduced red first, where build and parity both stayed green | `preservation-013-clobber-detected` eval |
