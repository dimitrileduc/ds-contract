...
<p>Emitted previews perceptually diffed against the design tool's own PNG renders — pixelmatch, with a text-masked second score because the design tool and the browser rasterize the same glyphs differently. Ranked <em>worst-first</em> as a standing fix queue; every row with a divergent (strictly positive) score carries a named cause from a committed triage table (contract-geometry / image-boundary / rendering / engine / instrument / figma-source) or prints <strong>UNTRIAGED</strong> — no amplitude dispenses a row from carrying one. A baseline gate re-scores every row and fails on regression beyond ±0.1pp; moving the baseline is an explicit, reviewed act. Reproduce: <code>npm run extract:figma:visual</code> → <a href="${REPO_URL}/blob/main/extract/figma/visual-parity/REPORT.md">REPORT.md</a>.</p>
