...
- [X] T102 [US2] Pose checkpoint `005/fix-design/tab-defaut`.
- [X] T103 [US2] Announce diff attendu = **visible sur les seules maquettes portant un Tab, à valider sur crop** — explicitly not zero-pixel, the iteration's one named exception (FR-015a); capture before ×9 → `.page-parity/V7/before/`. **⚠️ SUPERSEDED — investigation before any capture revealed the premise doesn't hold (see T104); no before-capture was taken since no gesture followed.**
- [X] T104 [US2] On `Tab` (`2061:1588`): remove the 2px underline from the `Défaut` variant so the `État` axis (already renamed in Phase 5/T040's sibling work — Tab's own axis was already named `État`) actually varies the rendering. **⚠️ MAJOR FINDING — NO GESTURE NEEDED. Investigated before writing anything: `Défaut` variant already has its bottom-stroke `visible: false`, `Sélectionné` already `visible: true` (confirmed via direct property read AND a screenshot of the live COMPONENT_SET — top variant shows no line, bottom shows one). Checked the Phase-5 archive clone (`Tab (archive avant suppression État3)`, cloned BEFORE any other edit in this spec) — same result on all 3 then-existing variants (Défaut/État3 invisible, Sélectionné visible). The `État` axis has varied the rendering correctly since before spec 005 touched anything. FR-015a's premise does not match the file at any point covered by this spec's history — no write performed, forcing one would be the opposite of the caution this exercise calls for.**
- [X] T105 [US2] Capture after ×9 → `.page-parity/V7/after/`, `pages:compare` → `proofs/V7`. Zoom into the crop on every page carrying a Tab and check every plausible property (weight, letter/paragraph spacing, borders) before accepting the diff as the expected one — never classify by color+position alone (the owner's standing lesson, two real bugs were found this way). **N/A — no gesture occurred, nothing to compare; verification was done via direct property read + screenshot + archive cross-check instead of a pixel-diff cycle.**
- [X] T106 [US2] Record V7 artifacts, close `decisions.md`, draft the gesture-record block — labeled explicitly as the iteration's one assumed design fix, never as zero-pixel. **Revised: zero design fixes occurred this iteration, not one — documented plainly in decisions.md, with the consequence flagged for T112 (cadence: 12 cycles consumed, not 13, under the SC-009 budget) and T111/RAPPORT-CLOTURE (no "assumed design fix" section needed).**
...
- [X] T107 Delete the `Archive · Spec A` page: capture before ×9 → `.page-parity/archive-deletion/before/`, delete the page, capture after ×9 → `.page-parity/archive-deletion/after/`, `pages:compare` → `proofs/archive-deletion`. Require 9/9 `identical` (SC-012) — verified, not assumed. **Done inside the owner-added ménage-final cycle (FR-030 groups zero-pixel gestures): before ×9 captured (archive-deletion/before, 9/9 manifests ok), Archive page confirmed (Tab clone + Footer clone) then deleted (6→5 pages), verdict shared with the ménage lot → proofs/menage-final (8/9 identical + the named 1px SAV residual — see decisions.md).**
- [X] T108 Run the full gate sweep **on the main checkout, not this worktree** (`npm run eval` symlinks `ROOT/node_modules` and cannot resolve it from a worktree): `npm run build && npm run parity && npm run eval && npm run plugin:check && npx tsx scripts/deterministic-roundtrip.mjs && node scripts/core-browser-check.mjs && npx tsc --noEmit && npx tsc -p tsconfig.build.json`. Expect strict status quo: 8/8 green, suite 108/108, `parity` zero active findings — any red is a regression and blocks closure (no exemptions are requested for this iteration, plan.md Constitution Check). **Done via 2 parallel Sonnet agents (owner-requested fan-out): build byte-identical/git clean, parity 0 active findings, tsc ×2 clean, plugin:check hash-conform, roundtrip byte-identical ×2, core-browser-check 4/4, eval 108/108 on the main checkout — 8/8 green, no exemptions.**
- [X] T109 [P] Compile the `RAPPORT-CLOTURE.md` **Divergences ouvertes** section: the Bouton contract↔source mismatch (FR-039, repair = major contract bump in Spec B) and the retained out-of-registry ghost icon (FR-038, repair = adopt-or-replace decision in Spec B). Both entries are mandatory — SC-017 blocks closure if either is missing. **Done (agent draft + merge): both mandatory entries + 4 more (Checkbox/icons zero-usage, Section-header FIXED root cause, Hero vidéo strate).**
- [X] T110 [P] Compile the **Valeurs laissées littérales** section from `releves/regle-3x-<date>.json`'s `<3` rows (T009/T032), each with its occurrence count (FR-012/SC-011). **Done — updated for the owner's end-of-iteration ≥2 rule: all former <3 chromatic values are now governed (noir-pur/gris-clair created; L2's T032 found checked-but-never-executed, caught and fixed); remaining literals = 5 named text nodes + 6 rich-text (B1), each with count and reason.**
- [X] T111 [P] Compile the **Dégradations & limites** section: every named degradation or limit surfaced across Phases 3–9 (the Tab underline fix, the Bouton divergence, any smaller/larger-than-predicted diffs, any masters excluded from a lot) — omission here is the highest-severity failure this iteration defines (FR-032/SC-010). **Done — includes the 3 process deviations, the closure incident (Accueil-copy misdiagnosis + variable-alpha API trap, repaired same evening), 3 prediction failures (cascade de centrage), L4's 3 echo regressions + 1px residual, the FIXED-children API limit (US7 0/6), the setRangeTextCase-detaches-style discovery, V7's false premise, the 7 content-derived names left out of scope.**
- [X] T112 [P] Compile the **Cadence** section: cycles actually consumed vs the 12-cycle budget (SC-009), naming any overrun at the point it was detected — never retroactively absorbed by merging two visual gestures. **Done: 13 consumed vs 12 (V7=0, ménage-final=+1 added by the owner at closure — overrun of 1, named at the moment it was decided, not absorbed).**
- [X] T113 Compile the **Compteurs de clôture** section, each counter checked against its opening relevé (T008/T009), never asserted without one. **Done — all counters sourced: 0 default names, 0 undescribed masters, 0 unpiloted hidden layers, 0 broken instances (157 re-resolved), 18+1 icons, Assets deleted, Archive deleted, Accueil-copy deleted, 0 ungoverned design colors, 11 declared literal texts, Organisms 15 ordered sections.**
- [X] T114 Assemble the full per-gesture quadruplet catalogue (`contracts/gesture-record.md` §2 format) — confirm 100% of gestures carry all 4 fields (SC-015). **Done: `RAPPORT-CLOTURE-quadruplets.md` (agent-compiled from all proofs/*/quadruplets.md, ordered, with an SC-015 field-presence check) + the ménage-final quadruplet in decisions.md.**
- [X] T115 Cross-check all 17 Success Criteria (SC-001…SC-017) against the assembled report and the T108 gate-sweep result; close any gap found before declaring the iteration done. **Done — see the check table in decisions.md § Clôture ; notable: SC-009 overrun of 1 named (owner-added 13th cycle), US7 delivered 0/6 with the API limit named (honest miss, not silent), everything else met.**
- [X] T116 Verify `decisions.md` is complete and internally consistent — every cycle entry (É, L1–L5, V1–V6, ménage-final) carries its diff attendu, diff observé, verdict, and `versionId`. **Verified — V7 documented as investigation-only (no gesture, premise false), every other cycle carries all four fields.**
