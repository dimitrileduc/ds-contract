...
- [ ] T063 **[NON FAIT sur le pilote, pour la même raison — les neuf pages sont construites et les liens éditeur fournis sur `piqueray-odoo-037` (8109), qui porte exactement ce module (`proofs/validations-owner.md`).]** [US4] Reconstruire les 9 pages sur le pilote (`npm run odoo:page -- <page> piqueray-odoo-pilote` ×9) et fournir à l'owner les liens **éditeur** `http://localhost:8087/odoo/website?path=<url>&enable_editor=1&with_loader=1` (mémoire projet : jamais le lien visiteur)
...
- [ ] T065 **[NON FAIT — aucune page n'a encore de verdict owner, donc aucune boucle de correction n'a eu lieu. La boucle est décrite et prête.]** [US4] Boucler chaque page renvoyée « à corriger » : correction du descripteur → recomposition (US1) → re-mesure (US3) → nouveau verdict daté, jamais l'inverse (FR-020)
