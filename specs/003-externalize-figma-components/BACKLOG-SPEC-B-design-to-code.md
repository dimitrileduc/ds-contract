...
| B5 | **Résolution des dépendances imbriquées par `contractId` d'abord, nom en repli** (~15 lignes dans `core/emit-figma-script.ts`) | `findComponentByName(spec.dep)` (`:3098-3106`, appelé `:3252`) cherche `n.name === name` et **lève** si introuvable — casse dès qu'un contrat dit `Button` et que le master vivant s'appelle « Bouton » (convention française du fichier). Constaté par la spec 006 (`ds.review-card`/`ds.google-reviews` : aucun `component`-ref vers `ds.button` posé, réemploi perdu et nommé — T034, `specs/006-google-reviews-block/research.md` R5). Non fait dans 006 : change les octets émis ⇒ churn de golden + éval (Principe II), hors périmètre déclaré |
| B6 | **`figma:plan` auto-nettoyant** pour les orphelins de `figma-sync/` | `scripts/generate-figma.ts:80-86` nomme `NN-<name>.js` par index positionnel et ne nettoie jamais les fichiers renumérotés — `golden-generated-output` itère les clés de golden, donc un orphelin reste conforme à golden pendant que le fichier réellement généré n'est plus épinglé (régression invisible). Purge manuelle faite dans 006 (6 orphelins, `git rm` + `golden:update` dans le même commit relu — T037, `specs/006-google-reviews-block/research.md` R13). Auto-nettoyage écarté : change le comportement émis ⇒ éval requise, hors périmètre |
