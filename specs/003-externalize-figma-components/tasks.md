...
- [X] T097 [US3] Master **Hero et catégories** (composite : instances de Hero T076 + Catégories T080, jamais des copies) → `audits/hero-et-categories.md` + entrée `decisions.md` — **fait 2026-07-25 : AUCUN master, par décision mesurée.** Le wrapper `Hero et catégories` (6 FRAMEs, tous sur `Pages`) est un cadre d'assemblage **sans identité visuelle propre** — VERTICAL, FIXED 1728 × HUG, gap **48**, padding 0, `fills/strokes/effects/radius` nuls, **identique sur les 6** ; hauteur = pure conséquence HUG (`Hero + 48 + Catégories`, vérifié 6/6). Il n'encode qu'un gap d'assemblage → même décision que `accordion` (T067) / `tabs` / `row`-Field. Décisif : Accueil porte **`Hero video`** (`210:330`, 720px negative-control hors périmètre), pas un Hero → un master baké serait faux ou hors-scope ; placement variable (5 enfants directs, 1 dans un GROUP `Header + Hero + Cat` sur Portes d'entrée) → aucun slot canonique. **Anti-fork vérifié : 0 COMPONENT/COMPONENT_SET `Hero et catégories` dans tout le fichier.**
- [X] T098 [US2] Adoption **Hero et catégories** — 6 pages → `ledger/hero-et-categories.json` + `proofs/hero-et-categories/README.md` — **fait 2026-07-25 : déjà adopté par T076 + T080, zéro mutation cette tâche.** Vérifié LIVE : 11 instances gouvernées dans les 6 wrappers (5 Hero `main 2111:3382` + 6 Catégories `set 2115:4277`), **0 copie brute**, **0 remote** (zéro tierce) ; seul enfant non-instance = `Hero video` d'Accueil (par conception). **Aucune preuve pixel avant/après** (aucune mutation, rien à capturer, R5/R8 sans objet) → preuve **structurelle + provenance** (plus forte que le pixel pour « instances gouvernées, zéro copie brute »). **Ledger vide explicite** (`entrees:[]`, `totaux 0/0`), `pages:ledger:check` **exit 0**. Points ouverts nommés (hors périmètre) : `Hero video` non externalisé, GROUP source superflu sur Portes d'entrée. Non committé.
- [X] T099 [US3] Master **Footer (+ Devis)** (composite : logo existant + Footer-column T058 + Copyright T060 + Bouton) → `audits/footer-devis.md` + entrée `decisions.md` — **fait 2026-07-25** : `COMPONENT` `2120:4785` (section `2120:4766` à `0,18050`), cloné du GROUP Footer de Contactez-nous ; **« + Devis » du nom = artefact, aucun contenu Devis réel dans le wrapper sur les 9 pages** (mesuré, pas supposé) ; Bouton variante « Outline blanc » vérifié blanc AVANT construction (précédent Hero/Devis) ; exécuté directement par l'orchestrateur après un 4e incident d'infrastructure de la nuit sur la tentative déléguée
- [X] T100 [US2] Adoption **Footer (+ Devis)** — 9 pages → `ledger/footer-devis.json` + `proofs/footer-devis/verdict.{json,md}` — **fait 2026-07-25** : 9/9 instances, **0 override** (contenu byte-identique sur les 9, cas le plus simple de la spec), bbox `{0,0,0,0}`, 0 copie brute, 0 tierce ; preuve pixel 9/9 diff (61-151px, ≪0,01%, bruit AA sous-pixel confiné au libellé du Bouton, crops inspectés) ; checkpoints posés AVANT chaque geste mutant
