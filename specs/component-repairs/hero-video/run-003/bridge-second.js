...
const comparableMemberFacts = (member, presentationValue) => {
  ...
  return { presentationValue, namesAndRoles, media, texts, componentProperties, sharedChildren };
};
const prepareResponsiveComponentSetLayout = (set, historical, host) => {
  // Native Figma refuses FILL/HUG sizing on variant members while their
  // COMPONENT_SET parent remains layoutMode=NONE. Make the additive set the
  // authoring auto-layout container before applying any declared member layout.
  // The set itself fills the already governed local Container; Pages and
  // consumer instances remain untouched.
  let changed = false;
  const assign = (field, value) => {
    if (!(field in set)) throw new Error('Responsive component-set layout field unsupported: ' + field);
    if (!sameValue(set[field], value)) { set[field] = value; changed = true; }
  };
  assign('layoutMode', 'VERTICAL');
  assign('primaryAxisSizingMode', 'AUTO');
  assign('counterAxisSizingMode', 'FIXED');
  assign('primaryAxisAlignItems', 'MIN');
  assign('counterAxisAlignItems', 'MIN');
  assign('paddingTop', 0); assign('paddingRight', 0); assign('paddingBottom', 0); assign('paddingLeft', 0);
  assign('itemSpacing', 0);
  if (Math.abs(set.width - historical.width) > 0.01) { set.resize(historical.width, set.height); changed = true; }
  if (host.layoutMode && host.layoutMode !== 'NONE' && 'layoutSizingHorizontal' in set) assign('layoutSizingHorizontal', 'FILL');
  return changed;
};
const applyResponsiveComponentSet = async (operation, target) => {
  ...
  if (prepareResponsiveComponentSetLayout(set, historical, host)) changed = true;
  for (const binding of responsive.primitiveBindings) {
    ...
    if (await applyPrimitiveBinding(member, binding, topology.historicalMember.presentationValue)) changed = true;
  }
  ...
  for (const layout of responsive.presentationLayouts) {
    ...
    if (applyPresentationLayout(member, layout)) changed = true;
  }
  for (const override of responsive.typographyOverrides) {
    ...
    if (await applyTypographyOverride(member, override)) changed = true;
  }
  ...
  return {
    ...
    result: !changed
      ...
      : { applied: true, createdNodeIds, createdNodes, changedNodeIds: [historical.id, host.id] },
  };
};
const scriptResults = [];
