// Exact rollback of the fully applied third attempt whose post-apply live
// inspection failed before a receipt could be returned.
await figma.loadAllPagesAsync();
if (figma.fileKey !== 'd9FYAUcqdcNtsuaMgLefvJ') throw new Error('H3 inspection recovery refused: wrong file');
...
if (!historical || historical.type !== 'COMPONENT' || historical.key !== '36011e51b8bc0b221a1ba6f9108709b5bd1c4490' ||
    !host || host.type !== 'FRAME' || !('children' in host)) throw new Error('H3 inspection recovery refused: historical/host identity drift');
const set = historical.parent;
if (!set || set.type !== 'COMPONENT_SET' || set.id !== '2580:6601' || set.name !== 'HeroVideo' || set.parent?.id !== host.id ||
    set.layoutMode !== 'VERTICAL' || host.children.length !== 1 || host.children[0].id !== set.id) {
  throw new Error('H3 inspection recovery refused: completed set signature absent');
}
const expectedMembers = new Map([
  ['2151:5552', 'Presentation=Wide'],
  ['2580:6587', 'Presentation=Compact'],
  ['2580:6594', 'Presentation=Desktop'],
]);
...
if (members.length !== 3 || members.some((node) => expectedMembers.get(node.id) !== node.name)) {
  throw new Error('H3 inspection recovery refused: completed members drifted');
}
const heightVariable = await figma.variables.getVariableByIdAsync('VariableID:2434:5919');
if (!heightVariable || heightVariable.name !== 'size/hero-video/root') throw new Error('H3 inspection recovery refused: height variable absent');
...
if (historical.parent?.id !== host.id || host.children.length !== 1 || host.children[0].id !== historical.id || historical.name !== 'HeroVideo' ||
  ...
  throw new Error('H3 inspection recovery postcondition failed');
}
return {
  schemaVersion: '1.0.0',
  run: 'interrupted-inspection-recovery',
  fileKey: figma.fileKey,
  recoveredAt: new Date().toISOString(),
  scriptResults: [{
    operationId: 'recover-interrupted-post-apply-inspection',
    status: 'recovered-to-fresh-standalone-baseline',
    historicalNodeId: historical.id,
    historicalComponentKey: historical.key,
    historicalHeightVariableId: heightBindingId,
    hostNodeId: host.id,
    removedCreatedNodeIds: ['2580:6601', '2580:6587', '2580:6594'],
  }],
  inspection: {
