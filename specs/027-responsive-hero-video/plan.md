...
> **Statut : Superseded — 2026-08-25.** Ce plan est conservé uniquement comme historique de la feature 027 arrêtée après H2. Il ne doit plus être exécuté. Le futur plan responsive Figma fera l’objet d’une spec séparée.
...
Faire de `ds.hero-video` le premier composant responsive Piqueray gouverné de bout en bout. L’exécution protège d’abord le master historique 1728 comme référence XL/wide et son unique usage Home (H1), puis fait choisir à l’owner les structures Mobile et Desktop dans un cadre déjà fixé : Mobile/compact sous 992 px, Desktop de 992 à 1399 px et XL/wide à partir de 1400 px (H2). La largeur Tablet 834 reste un témoin obligatoire de la composition compacte, sans quatrième composition. H2 s’arrête au layout ; une décision transverse approuvée doit ensuite fournir spacing, typographie et valeurs d’enfants avant que la source Figma puisse être adaptée et acceptée (H3), puis promue dans un modèle contractuel générique et déterministe.
...
**Constraints**: four explicit human gates; H2 layout acceptance alone authorizes no source mutation; an approved transverse foundation decision for spacing, typography and child components is required before Figma work; no cross-surface side-sync; no generated-file hand edit; no public/editor `viewport` control; historical XL reference 1728×720 and wide continuity at 1440 preserved; Mobile/compact `<992`, Desktop `992–1399`, XL/wide `>=1400`; Tablet 834 maps to compact; Figma resize is not claimed to trigger a breakpoint; saved Odoo HTML is never silently rewritten; Header is read-only context; Odoo/Bootstrap globals and `.o_pqr_page` are not overridden
...
**Post-design re-check**: passed. The design separates the owner-approved layout direction from the future transverse values, preserves the settled 992/1400 profile and four witnesses, introduces only an optional generic capability, preserves the contract as the propagation point, and gives every destructive or persistent-state risk a refusal gate. No constitutional waiver is required. The active Superset worktree is the isolated implementation target and no sibling worktree is created or selected.
...
2. Utiliser le worktree Superset actif comme frontière autosuffisante, y exécuter `npm install` et `npx playwright install chromium`, vérifier que la baseline de parité visuelle y est versionnée, puis épingler le commit, le tree Git et la version Figma avant de calibrer les transports/captures sans mutation.
...
### Phase B — Co-concevoir le layout Mobile/Desktop et figer H2

1. Produire au moins deux options comparables Mobile/Desktop dans un harness local non autoritatif, sans écrire la source Figma. Chaque option renseigne présence, ordre DOM/visuel, axe, alignements, stratégie de hauteur, contraintes de placement, rôle Text Style conservé, placement du CTA actuel, média et repli faible hauteur ; XL reste le baseline actuel.
2. Tester chaque option avec le contenu par défaut et long aux témoins 390, 834, 1200 et 1728, à 320, en paysage court, puis autour de 992 et 1400. Le harness peut faire varier padding, gap et typographie pour éprouver le layout, mais chaque valeur reste une preview explicitement non autoritative. La largeur 834 utilise compact ; si elle ne respecte pas les critères structurels, revenir devant l’owner plutôt que créer silencieusement Tablet.
3. Décrire le mécanisme structurel probable : Auto Layout si la composition reste identique et variant uniquement pour une vraie différence de présence, ordre, parenté ou axe. Les modes de valeurs restent hors de H2. Les trois compositions runtime sont `compact`, `desktop` et `wide`; aucun variant Tablet séparé n’est créé.
4. Enregistrer H2 : option de layout retenue, profil 992/1400, limites, décideur, date, preuves et sujets transverses différés. H2 autorise uniquement la préparation de la spec transverse de fondation. Aucune campagne Figma, promotion de contrat ou adaptation web/Odoo ne peut commencer sur la base des previews.

### Phase C — Adapter la source et obtenir H3

1. Reprendre cette phase seulement après qu’une spec transverse approuvée fournit les valeurs exactes de spacing, typographie et enfants nécessaires au layout H2. Lier son reçu à la campagne, puis immédiatement avant toute écriture Figma — et de nouveau avant chaque écriture ultérieure — réauditer le master et tous les usages par identité et position. Si une source non propre est observée, arrêter la campagne ; elle est nettoyée uniquement avec une autorisation owner distincte, puis réauditée.
...
5. Capturer après, vérifier texte long, CTA actuel, média, faible hauteur, liens, overrides et absence de delta XL inattendu, puis enregistrer H3. Une observation contraire au layout H2 ou aux valeurs de la décision transverse revient devant l’owner.
...
2. Ajouter au schéma un bloc `responsive` optionnel et générique. Il porte `basis: viewport-width`, une copie gouvernée des seuils, des compositions ordonnées par `minWidthPx`, des témoins Design séparés et des overrides bornés par part ; il réutilise le vocabulaire layout/token/literal existant et nomme les canaux code-only. HeroVideo y transporte la structure H2 et uniquement les valeurs fournies par la décision transverse liée, jamais des valeurs locales improvisées. Il ne devient ni une prop métier ni un réglage Odoo. Les nombres du contrat restent l’autorité, la référence Odoo n’est qu’une provenance vérifiée.
3. Compiler cette source par un lowering partagé : base compact puis `@media (min-width: 992px)` et `@media (min-width: 1400px)` pour HTML, CSS Modules React, référence inline, Web Components et CSS Odoo. L’inline déplace tout canal responsive dans un `<style>` déterministe et scoppé, ou refuse explicitement le contrat ; aucune propriété inline ne doit neutraliser les media queries. Figma projette la structure H2 et les valeurs transverses approuvées, avec annotation honnête de ses limites de canvas.
...
2. Préserver le DOM QWeb actuel : les mêmes parts BEM et contrôles de poster, alt, titre, libellé/href CTA reçoivent la CSS responsive générée. Le Button garde un seul nœud focusable, sa largeur intrinsèque et ses valeurs internes ; un éventuel marqueur stable ne peut porter que son placement par la section. Aucun CTA dupliqué ni `order` CSS ne peut désynchroniser l’ordre visuel et l’ordre de lecture. Le canal vidéo reste code-only et le poster reste le repli Odoo/Figma nommé.
