/**
 * Spike de mécanisme Texte SEO — AVANT tout polish (guide canonique, étape 3).
 *
...
async function main() {
  ...
  const receipt = new Recueil('texte-seo-mechanism-spike', 'odoo-019-foundation', 'texte-seo-contract-sample');
  if (!dockerDisponible()) {
    receipt.saute('spike Texte SEO sur Odoo', 'Docker indisponible', 'ODOO-LIMIT-NO-INSTANCE');
    receipt.ecrire('texte-seo-mechanism-spike.json', Date.now() - started);
    process.exit(1);
  }
  ...
  const done = receipt.ecrire('texte-seo-mechanism-spike.json', Date.now() - started);
