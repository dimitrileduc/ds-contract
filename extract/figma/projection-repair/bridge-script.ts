...
export function validateBridgeOperation(operation: PlannedOperation): string[] {
  ...
  if (operation.mechanism === 'responsive-component-set') {
    return operation.changes.capability === 'responsive-component-set' && Object.keys(operation.changes).length === 1
      ? []
      : ['responsive-operation-not-allowlisted: responsive-component-set requires the closed generic capability marker'];
  }
...
export function emitBridgeApplyScript(campaign: RepairCampaign, plan: DryRun, run: 'first' | 'second'): string {
  ...
  const payload = {
    ...
    responsive: target.responsive ?? null,
    })),
