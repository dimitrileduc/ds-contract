...
/** Emit the bounded Plugin-API transport for generic workflow operations.
 * Component-specific anatomy remains in generated component scripts; this
 * transport only owns the reusable organism Container presentation rule. */
export function emitBridgeApplyScript(campaign: RepairCampaign, plan: DryRun, run: 'first' | 'second'): string {
  const unsupported = plan.operations.filter((operation) => operation.mechanism !== 'ensure-organism-container');
  if (unsupported.length > 0) throw new Error(`bridge emitter does not own: ${unsupported.map((entry) => entry.mechanism).join(', ')}`);
  const payload = {
    schemaVersion: '1.0.0',
