...
for (const file of readdirSync(DECL).filter((f) => f.endsWith('.json') && !f.startsWith('_')).sort()) {
  ...
  // La fusion CORRIGE, elle ne fait pas que remplir un trou.  Tant qu'un sujet
  // déjà porteur de faits était sauté, une déclaration erronée n'avait plus
  // aucun chemin officiel de correction — c'est précisément ce qui a fait
  // naître le canal parallèle `out/overrides/` (gitignoré), et donc des preuves
  // commitées irreproductibles depuis les entrées commitées.
  //
  // Ce que la déclaration ne porte PAS, elle ne l'efface pas : les entrées de
  // comparaison (`reactProps`, `fixtureAssetIds`, `fixtureNote`) vivent dans le
  // manifeste et sont conservées, sinon la fusion re-fabriquerait le trou.
  const previousCase = subject.cases?.[0];
  const isCorrection = Boolean(subject.facts?.length);
  ...
  if (decl.case && previousCase) {
    for (const key of ['reactProps', 'fixtureAssetIds', 'fixtureNote']) {
      if (previousCase[key] !== undefined && decl.case[key] === undefined) {
        subject.cases[0][key] = previousCase[key];
      }
    }
  }
  subject.coverage.requiredFactIds = decl.facts.map((f: any) => f.id);
  console.log(
    `${isCorrection ? '~' : '+'} ${id} : ${decl.facts.length} faits, ${subject.cases.length} cas` +
      (isCorrection ? ' (CORRIGÉ — entrées de comparaison conservées)' : '') +
