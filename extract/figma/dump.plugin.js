...
//   vectorAsset        monochrome arbitrary SVG export (dump v1.8): asset name,
//                      intrinsic rectangle, local position, and base64 transport
//                      bytes; propose.ts promotes the bytes to assets/vectors/
...
const TARGET_SETS = ['Badge', 'Switch', 'Card', 'PiquerayLogo'];
...
// Parametric decor remains the v1.3 shape path. Arbitrary-path vectors use
// the v1.8 monochrome SVG promotion path below; failed promotions retain a
// named #42 degradation receipt.
...
// Arbitrary SVG promotion (dump v1.8): a single vector, or a GROUP composed
// entirely of vectors, carries its Figma-exported bytes as transport evidence.
// The contract proposer emits only the external asset name + geometry; SVG path
// data is never written into a contract.
const assetSlug = (s) => s
  ...
  .replace(/^-+|-+$/g, '')
  .toLowerCase();
const vectorLeaves = (node) => {
  const out = [];
  const walk = (n) => {
    if (VECTOR_TYPES.includes(n.type)) { out.push(n); return; }
    for (const child of n.children || []) walk(child);
  };
  walk(node);
  return out;
};
const hasOnlyVectorDescendants = (node) => {
  if (VECTOR_TYPES.includes(node.type)) return true;
  const children = node.children || [];
  return children.length > 0 && children.every((child) => hasOnlyVectorDescendants(child));
};
const isPromotableVector = (node) =>
  VECTOR_TYPES.includes(node.type) ||
  (node.type === 'GROUP' && hasOnlyVectorDescendants(node));
...
/** Promote source geometry only when every drawable leaf has the SAME solid
 * paint. Multicolour/mixed vectors must stay an explicit receipt: converting
 * them to one currentColor asset would silently change their picture. */
async function dumpVectorAsset(node, nodePath) {
  if (!isPromotableVector(node)) return null;
  const leaves = vectorLeaves(node);
  const paints = [];
  for (const leaf of leaves) {
    const paint = await dumpPaint(leaf.fills, nodePath, 'fill', leaf);
    if (!paint) {
      degrade('vector-asset-promotion-refused', nodePath, 'vector asset has a non-solid, invisible, or absent leaf fill — SVG export not promoted as a currentColor asset');
      return null;
    }
    paints.push(paint);
  }
  const keys = new Set(paints.map((p) => JSON.stringify(p)));
  if (keys.size !== 1) {
    degrade('vector-asset-promotion-refused', nodePath, 'vector asset has mixed leaf paints — SVG export not promoted as a single currentColor asset');
    return null;
  }
  const componentName = nodePath.split(':')[0];
  const bytes = await node.exportAsync({ format: 'SVG' });
  return {
    asset: assetSlug(componentName) + '-' + assetSlug(node.name),
    width: node.width,
    height: node.height,
    ...(typeof node.x === 'number' && typeof node.y === 'number' ? { position: { x: node.x, y: node.y } } : {}),
    svgBase64: figma.base64Encode(bytes),
    paint: paints[0],
  };
}
...
async function dumpNode(node, nodePath, parent) {
  ...
  // sizing evidence before fixing a dimension (root FIXED axis, INSTANCE
  // geometry, or TEXT autoResize NONE), so this is measurement, not a guess.
  ...
  const capturedVector = shape ? null : await dumpVectorAsset(node, nodePath);
  if (capturedVector) {
    const { paint, ...vectorAsset } = capturedVector;
    out.vectorAsset = vectorAsset;
    // GROUPs have no fill of their own; carry the verified uniform leaf paint
    // on the promoted part so the proposer can bind SVG currentColor.
    out.fill = paint;
  }
  ...
  if (VECTOR_TYPES.indexOf(node.type) >= 0 && !capturedVector) {
    degrade('vector-geometry-unsupported', nodePath, node.type + ' geometry could not be promoted as a monochrome external SVG asset — see the vector-asset-promotion-refused receipt; this node carries paints only and renders as a box');
  }
  ...
  if (stroke) {
    ...
    if (typeof node.strokeWeight === 'number') out.strokeWeight = node.strokeWeight;
      ...
      // dump v1.7: Figma exposes each side even when the shorthand is MIXED.
      // Preserve all four literals (including zeroes) and let `bound` retain
      // an independently bound side. This is generic per-side stroke support,
      // not a component-specific exception.
      out.strokeWeights = {
  ...
  if (node.type === 'TEXT') {
    ...
    if (
      ...
      channels.push('letterSpacing ' + node.letterSpacing.value + node.letterSpacing.unit + ' — nonzero percent spacing is not yet carried');
    }
    ...
    if (node.fontName !== figma.mixed) text.fontFamily = node.fontName.family;
    if (node.textAutoResize) text.autoResize = node.textAutoResize;
    if (
      node.letterSpacing !== figma.mixed &&
      node.letterSpacing &&
      (node.letterSpacing.value === 0 || node.letterSpacing.unit === 'PIXELS')
  ...
  if (node.type === 'INSTANCE') {
    ...
    try {
      ...
      for (const [key, def] of Object.entries(node.componentProperties || {})) {
        // Applied INSTANCE_SWAP values configure the nested child itself;
        // they are not parent slots. Resolve the session-local component id
        // to its stable drawn name so the proposer can invert it through the
        // child contract's bindings.figma.values.
        if (def.type === 'INSTANCE_SWAP') {
          const selected = typeof def.value === 'string' ? await figma.getNodeByIdAsync(def.value) : null;
          if (selected && selected.type === 'COMPONENT') {
            ...
            degrade(
              ...
              nodePath,
              'INSTANCE_SWAP property "' + key.split('#')[0] + '" value ' + String(def.value) + ' did not resolve to a COMPONENT — raw value preserved',
            );
          }
          continue;
        }
  ...
  if ('children' in node && node.type !== 'INSTANCE' && !capturedVector) {
    out.children = [];
