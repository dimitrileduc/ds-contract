...
| — | ~~89~~ **77 littéraux** de typographie, peinture et trait sont hors du périmètre de `geometry:gate` — donc invisibles à l'axe qui gouverne. **Le mécanisme, lui, EXISTE déjà** : registre fermé `contracts/named-literals.registry.json` + porte fail-closed. Ce qui les laisse passer n'est pas un manque d'outil mais une **décision de périmètre écrite dans le code** (`extract/geometry-gate/gate.ts` §`GEOMETRIC_CHANNELS` : « Paint …, trait …, typography … are OUT OF POPULATION — widening this set is a versioned evolution of the interface, never a silent adjustment »). | **Bien plus petit qu'une spec type 015 — chiffré le 2026-08-07.** 015 devait *construire* la porte et minter 83 références ; ici la mécanique ET le vocabulaire existent (72 feuilles `typography.*`, 29 `font.*`, 15 `color.*`, 3 `border-width.*`). Travail réel : élargir la liste de 12 canaux, versionner `contracts/geometry-gate.interface.md` §2, puis pointer 77 valeurs — **65 ont déjà un token de valeur identique, 12 n'en ont aucun** (à minter from-dump ou à déclarer au registre). ⚠️ **Piège nommé** : une valeur identique n'est pas le bon token — le rapprochement propose `line-height: 24px → {font.size.24}` et `letter-spacing: 0px → {space.0}`, coïncidences de nombre sans rapport de sens. Les 65 ont un *candidat*, pas une réponse : c'est du jugement, pas de la substitution |
