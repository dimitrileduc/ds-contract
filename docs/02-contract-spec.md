...
**Rich text and `marks.strong`.** A `rich-text` prop is a non-empty array of `{ text, strong? }` segments; it binds to one native Figma `TEXT` property when the set exposes one, or to `NONE` when the text lives on the node (owner rule, 2026-09-02: what makes a text rich is its content — a line break or a bold range — never whether the canvas exposes a property). The canvas receives the concatenated text while code preserves segment boundaries as semantic `<strong>` ranges; a line separator inside a segment is a governed break (emission normalizes Figma's `\n` / `\r` / U+2028 to `\n`, rendered under `white-space: pre-line`). A content part may govern those ranges with `marks.strong` as either a legacy weight token reference or `{ "font-weight": token-ref | "100"…"900", "font-size"?, "line-height"? }`. The optional size and leading are bounded literals; the weight is always required, so browser-default bold is never an unstated style fact.
