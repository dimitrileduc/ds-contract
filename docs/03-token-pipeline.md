...
- **The thresholds are tokens**: `breakpoint.tablette = 768px`, `breakpoint.desktop = 992px`, `breakpoint.wide = 2000px` (primitives). 992 and 768 are the Odoo 19 / Bootstrap 5.3 grid steps lg and md (992 decided in spec 027 R6; 768 added for the Tablette planche validated by 031). **`wide` was 1400 — Bootstrap's `xxl` — until 2026-09-08, and the move off it is a deliberate departure from that referential, named here because the sentence above used to claim the profile was a pure Bootstrap subset.** Bootstrap has **no step above 1400**, so there was no next Bootstrap value to take; 1600 is Ant Design's `xxl` and the entry of Material Design 3's Extra-Large class, whose Large class (1200–1599) covers our desktop witness of 1200 exactly. Measured cause: at 1400, a MacBook Air 13" (1470 CSS px) and a MacBook Pro 14" (1512) both fell into `wide` and received the composition drawn for **1728** — the H1 rendered at 54px, the page gap at 192, the section gutter at 89, on screens 200–260px narrower than the drawing. At 1600 they receive the 1200 composition (40 / 128 / 56) and a MacBook Pro 16" — whose CSS viewport is **exactly 1728**, the wide witness width — keeps `wide` with 128px of margin. 1728 itself was rejected as the threshold: a breakpoint belongs **between** two device families, never on one of them, or the very machine the mode was drawn for sits on the edge and leaves it on any zoom or non-maximised window. The build **inlines** them into the `@media` conditions and never emits them as custom properties: CSS cannot read a `var()` inside a media query. A language limit, named. **`wide` moved again on 2026-09-16, 1600 → 2000 (owner decision).** The measured consequence is the opposite of the 2026-09-08 one and is accepted: **no laptop receives the 1728 composition any more** — a MacBook Pro 16" (exactly 1728 CSS px) now gets the 1200 composition — and `wide` only serves external monitors (27" QHD, 4K at 150 %, ≥ 2000 CSS px). 2000 is the step of no referential (Bootstrap, Ant Design, Material 3); it is a threshold placed **between** the laptop family (≤ 1728) and the external-monitor family. The 18 hand-written responsive CSS files of the Odoo module (`responsive.pqr.css`, `odoo-bridge.css`, `responsive/*.pqr.css`) repeat the value literally — a media query cannot read `var()` — and no gate checks that they follow the token; each move is a `sed` and a `grep` (tinyspec `breakpoint-wide-2000.md`).
...
@media (min-width: 2000px) { :root { --typography-h1-size: var(--font-size-54); … } }
```
