...
**L1 — WITHDRAWN 2026-09-05. It was wrong, and it was written here before it was tested.**

The original text claimed a state could not vary by enum value, so underlining the `link` button style would underline all seven. **That is false**, and the mistake is instructive enough to leave visible rather than delete.

What the engine actually does: when a state's value carries a `{prop}` placeholder, `emit-react.ts` (root-states loop) resolves the enum and emits **one rule per value** — `.variant-link:hover`, `.variant-default:hover`, and so on. State rules have been per-enum-value all along.

The real constraint was narrower and had nothing to do with the state channel: an underline is a **keyword**, not a colour, so it needed a token that could carry a keyword. The repo already had one — `nav.state`, a `$type: "string"` token that reaches CSS as a custom property. So the recipe is ordinary: mint one `string` token per enum value (`underline` for `link`, an explicit `none` for the other six), and bind `text-decoration-line` in the hover state. Spec 032 shipped exactly that in `ds.button` 2.3.0; the emitted CSS is `.button--variant-link:hover:not(:disabled) { text-decoration-line: var(--decoration-etat-link-survol); }`.

The explicit `none` on the six other styles is not decoration: it is what makes the governed value win over a host rule that would underline our anchors (Odoo's `a:hover` does exactly that).

**Why this row now reads as a receipt rather than a limit.** L1 was written from reasoning about the schema shape, not from running the emitter. It then travelled — into the contract's own description, into the spec, into the closure report — and it took an owner asking "why exactly is it impossible?" to overturn it in one trial. **A limit claimed without a fixture is not a limit; it is a guess with a citation.** The order this repo requires is fixture → eval → claim, and it applies to declaring what is impossible exactly as much as to declaring what works.
...
*Addendum (2026-07-26, 006-google-reviews-block): **A5 (`background-image: url()`, row 91 above) stays an open named gap — not closed by this spec.** `ds.review-card`'s `avatarPhoto` part (element `img`) carries `photoUrl` end-to-end in the contract and code emitter (`img` renders the real bitmap), but the canvas compiles it to `imgPlaceholder:true` + the standard `#D9D9D9` wash — the real pixel stays an out-of-contract Figma override (T029, T066; demonstrated live in `specs/006-google-reviews-block/proofs/us4-proprietes/`, code renders the photo, canvas shows an empty circle). `figma.createImage` remains native and unused by this spec; A5's "add" verdict (§ a.7) is unaffected.
**New, complementary to the matrix (not a CSS channel, so no table row)**: a **region-based proof** class was added to `extract/figma/page-parity/cli.ts`'s `--regions` flag (fixture → eval `pages-compare-regions-additive`, T029, before any doc claim — see `specs/006-google-reviews-block/contracts/region-proof.md` for the full method). It restricts a pixelmatch diff to a named rectangle inside a page-sized capture and reports `regionDiffCount`/`regionPct` (inside the rectangle) alongside `outsideDiffCount` (everywhere else) — the tool that let 006 claim "the block matches its region" and "nothing outside the block moved" (SC-002/SC-003) on maquettes where the region is a small fraction of the full page. It is a proof-methodology addition to the page-parity instrument, not a new capability row in this matrix (it changes nothing about what Figma nodes can express).*

*Addendum (2026-08-03, **what a named limit means at REGENERATION time** — the question this document could not answer until now): a named limit is not merely "the contract does not carry it". It also decides **who owns the pixel when the canvas is rebuilt**, and that answer lived only in code comments and one eval header — nowhere in `docs/`. Stated here because it is the first thing anyone asks about a limit, and the second thing they get wrong.*

*The rule, in two lines:*

| what the designer changed on canvas | on regeneration |
|---|---|
| a fact the contract **can** carry (a gap, a colour, a text) | **overwritten, deliberately** — `amendSet`/`amendComponent` rebuild variant interiors from the spec; manual interior edits are drift by definition |
| a fact the contract **cannot** carry (A5: the real photo) | **preserved**, by an explicit rescue pass — never by luck |

*The rescue (`core/emit-figma-script.ts`): `harvestImagePaints` collects every `IMAGE` paint from the existing tree **before** the rebuild; `restoreImagePaints` re-applies them **after**, matched by part name, with an order-based fallback that **rehouses** a root-level photo onto the first unclaimed `img` part (hero's real shape: the photo is a root fill while the contract models a Background child). Every paint preserved, rehoused, or **unplaced** is named in the amend report (`preservedImages` / `unplacedImages`) — never silent. Without this pass, a re-sync would destroy the client's photos on all **9 image-bearing Piqueray components** and replace them with the `#D9D9D9` wash. Guarded by eval `img-paint-preserved-on-amend` (three cases: name-matched, rehoused-from-root, fresh-create keeps the placeholder).*

***Two limits of the rescue, named because they are live risks for spec 016** (which regenerates the divergent canvas):*
1. *A paint with **no** placeable `img` part goes to `unplacedImages` — reported, but **not restored**. It is lost, loudly.*
...
*1. **Composed-instance slot content is a contract channel** (`component.slots`, schema v20 — a composition fact, not a CSS channel, so no table row): a `component` part can state what the composed child's INSTANCE_SWAP slot carries; the canvas emitter lowers it to `depSlots` and sets the child's swap componentProperty with the slotted component's id, resolved by the `ds_contracts/contractId` marker — never a manual instance override. Measured origin (Formulaire row5, 2026-08-06): the master's Field slot held a TEXTAREA (« Message », h=161) as a manual override inside a governed master, and every parent amend erased it (the D7 family — INSTANCE_SWAP lost where TEXT survives; measured: row5 h=81, the default input, page contactez-nous −58). Fixture, green: `evals/fixtures/composed-child-slot-content-check.ts` — the schema parses the channel, the emitted script carries the marker-resolved swap (`depSlots`: property + depId + props mapped through the slotted contract's own bindings), the mock-built composed instance carries the slot's INSTANCE_SWAP prop with the slotted component's id, and a witness composition without `slots` sets no swap prop.*

*2. **Height-0 + per-side stroke = LINE — the §1 `box-sizing` row's INSIDE doctrine gains its named exception**: Figma clamps a frame's height to the total of its horizontal strokes when `strokeAlign` is INSIDE — measured live on Footer.Separator (origin: a LINE 1550×0, the stroke is dressing, flow contribution 0; the emitted INSIDE frame landed h=2 with inherited uniform weights, h=1 at best — +2 invisible px on every page). The rule: a part with height 0 carrying a per-side stroke is a line, not a box → `strokeAlign: 'CENTER'` (the align of a LINE's own stroke, which does not clamp) + re-resize after the weights are set; any non-zero-height part with the same stroke keeps INSIDE (the 015 border-box doctrine — right for boxes). Fixture, green: `evals/fixtures/zero-height-line-part-check.ts` — the mock now reproduces the measured INSIDE clamp at `resize` AND at weight pose, the line lands < 0.5 tall, and the witness box keeps INSIDE and its height.*
