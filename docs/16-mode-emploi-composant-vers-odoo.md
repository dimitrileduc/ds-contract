...
| Formulaire | accroche, titre (gras), consentement, avantages (titre, texte) | le formulaire lui-même (champs, étiquettes, bouton) | les options NATIVES de champ/étiquette Odoo sont exclues sous `.s_pqr_formulaire` (EB-019, `excludeNativeFormOptionsForRoots`) — sinon un clic sur une étiquette ouvre « Oops » et fige l'éditeur. Reste : pas d'ajout d'avantage ; message de succès `hidden` inéditable |
